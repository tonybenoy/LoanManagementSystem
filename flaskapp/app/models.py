...
TYPE = {"customer": 0, "agent": 1, "admin": 2}

STATE = {"New": 0, "Rejected": 1, "Approved": 2}


class Loan(db.Model):  # Defining the sql alchemy model
    id = db.Column(db.Integer, primary_key=True)  # Defining the fields
    ...
    def emicalc(self):  # Calc from input
        self.emi = (
            self.principle * (math.pow((1 + self.roi / 100), self.tenure))
            - self.principle
        ) / self.tenure
        self.total_amount = self.principle * (
            math.pow((1 + self.roi / 100), self.tenure)
        )

    def createuid(self, userid):  # Assign creator id
        self.create_uid = userid

    def __repr__(self):  # How to show oject
        return "<Loan {}>".format(self.id)


class LoanSchema(ma.ModelSchema):  # Marshmallow schema class
    class Meta:
        model = Loan
        include_fk = True  # Include foreign key


class LoanRollback(db.Model):  # History model
...
class User(UserMixin, db.Model):
    ...
    @login.user_loader  # Add user to session
    def load_user(id):
