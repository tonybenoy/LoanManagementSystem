...
class FlaskTestCase(BaseTest):
    ...
    def test_login_page(self):
        response = self.client.get('/login',content_type='html/text', follow_redirects=True)
    ...
    def test_register_app(self):
        with self.client as c:
            response = c.post('/register',
             data=dict(username="admin2", email="admin2@test.com",password="admin2",passwordrep="admin2"),
              follow_redirects=True)
            self.assertTrue(b"Congratulations, you are now a registered user!" in response.data)

    def test_register_api(self):
        with self.client as c:
            response = c.post('/' + app.config["API_FOR"] + '/' + app.config["API_VERSION"] + "/user",
             data=dict(username="admin2", email="admin2@test.com",password="admin2"),
              follow_redirects=True)
            self.assertTrue({"message": "User admin2 created"} , response.data)
     
    def test_login_api(self):
        with self.client as c:
            response = c.get('/' + app.config["API_FOR"] + '/' + app.config["API_VERSION"] + "/login",
    ...
    def test_users_api(self):
        with self.client as c:
            ...
            response = c.get('/' + app.config["API_FOR"] + '/' + app.config["API_VERSION"] + "/users",headers={'x-access-token':key}  ,follow_redirects=True)
            self.assertEqual(response.status_code, 200)
        ...
        with self.client as c:
            response = c.get('/' + app.config["API_FOR"] + '/' + app.config["API_VERSION"] + "/login",
             headers={'Authorization': _basic_auth_str("admin", "admin")},
              follow_redirects=True)
            key = json.loads(response.data)["token"]
            response = c.get('/' + app.config["API_FOR"] + '/' + app.config["API_VERSION"] + "/loans",headers={'x-access-token':key}  ,follow_redirects=True)
            self.assertEqual(response.status_code, 200)
            import pdb
            pdb.set_trace()
