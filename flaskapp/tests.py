...
class BaseTest(TestCase):
    ...
    def setUp(self):
        ...
        user = User(username="customer1",
                    email="customer1@test.com", type_of_user=0)
        user.set_password(password="customer1")
        db.session.add(user)
        user = User(username="customer2",
                    email="customer2@test.com", type_of_user=0)
...
class FlaskTestCase(BaseTest):
    ...
    def test_register_app(self):
        with self.client as c:
            response = c.post('/register',
                              data=dict(username="admin2", email="admin2@test.com",
                                        password="admin2", passwordrep="admin2"),
                              follow_redirects=True)
            self.assertTrue(
    ...
    def test_users_api(self):
        with self.client as c:
            ...
            response = c.get('/' + app.config["API_FOR"] + '/' + app.config["API_VERSION"] +
                             "/users", headers={'x-access-token': key}, follow_redirects=True)
            self.assertEqual(response.status_code, 200)
    ...
    def test_loans_api(self):
        with self.client as c:
            ...
            key = json.loads(response.data)["token"]
            response = c.get('/' + app.config["API_FOR"] + '/' + app.config["API_VERSION"] +
                             "/loans", headers={'x-access-token': key}, follow_redirects=True)
            self.assertEqual(response.status_code, 200)
